...
# 【核心修复】统一多语言状态键为 lang，初始化并兼容旧状态
if 'lang' not in st.session_state:
    # 若原有 language 状态存在，迁移值；否则检测浏览器语言
    st.session_state.lang = st.session_state.get('language', detect_browser_language())
# 初始化打赏相关状态
...
lang_texts = {
    'zh': {
        ...
        'games': [
            ...
            ("为什么要抓马杜罗", "委内瑞拉的石油和毒品", "🛢️", "https://venezuela.streamlit.app/"),
            ("MBTI对话助手", "用AI对付MBTI", "🧠", "https://mbtibot.streamlit.app/"),
            ("MBTI亿万富翁", "我的性格怎么发财", "🧬", "https://1000000.streamlit.app/")
        ]
    },
    'en': {
        'page_title': 'AI.Fun',
...
# 【统一】多语言文本获取函数（所有地方均调用此函数）
def get_txt(key):
    return lang_texts[st.session_state.lang][key]
# 【辅助】当前语言文本快捷变量（用于非打赏区域，如首页、页脚）
current_text = lang_texts[st.session_state.lang]

# ==========================================
# 4. 核心 CSS (保留所有原有样式，无修改)
# ==========================================
st.markdown(f"""
<style>
    /* 基础重置 */
...
def render_home():
    ...
    with t_col2:
        ...
        with inner_col1:
            # 语言切换按钮 - 统一更新 lang 状态
            l_btn = "En" if st.session_state.lang == 'zh' else "中"
            if st.button(l_btn):
